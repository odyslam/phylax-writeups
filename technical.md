...
Ideally, the blockchain clients expose health endpoints via HTTP such that we can implement intelligent Liveness and Readiness checks. Liveness would correspond to a `/health` endpoint that returns HTTP 200 when the process is running, and Readiness would manifest in the form of a script that checks the reported sync state of the node, checks the reported block height against third-party block explorers or other nodes on the cluster, perhaps the number of connected peers, and any other KPIs identified for the protocol.
